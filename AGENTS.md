# AGENTS.md — agent rules for Fronts of War

This is the Codex-convention twin of `CLAUDE.md` in this repo's root — same
ruleset, kept in sync by hand. If you're an agent reading this file, treat
it as fully authoritative; you don't need to also read `CLAUDE.md`. If
you're editing one of these two files, edit the other to match.
...
(`User` if they said so explicitly, `Claude`/`Codex` if you decided it under
...
calls, and this file is how the next session (or the next agent, possibly a
different tool entirely) avoids re-litigating or accidentally reversing
something.
