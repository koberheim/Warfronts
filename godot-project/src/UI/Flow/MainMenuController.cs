...
// war table with Campaign as the one primary action. Skirmish, Endless and
// Codex are shown locked until their ladder prompts land (D54); Settings
// unlocked in D80. The right two-thirds of the table stays clear for the
// campaign map.
public partial class MainMenuController : Node2D
{
    public override void _Ready()
    {
        ...
        var campaign = UiFactory.Button("PrimaryButton", "Campaign", () => GetTree().ChangeSceneToFile("res://scenes_root/campaign_selection.tscn"));
        campaign.CustomMinimumSize = new Vector2(0f, 52f);
        column.AddChild(campaign);

        var settings = UiFactory.Button("PaperButton", "Settings", () => GetTree().ChangeSceneToFile("res://scenes_root/settings.tscn"));
        settings.CustomMinimumSize = new Vector2(0f, 44f);
        column.AddChild(settings);

        foreach (var name in new[] { "Skirmish", "Endless", "Codex" })
        {
        ...
        column.AddChild(UiFactory.Wrapped("PaperSmallLabel", "Skirmish, Endless and the Codex arrive later in development."));
